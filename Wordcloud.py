...
st.markdown("""
    ...
    <p style="font-size:16px; display:inline; color:gray;">   ~By Mugesh Murugaiyan.</p>
...
st.markdown("---")
st.markdown("Created by: [Mugesh Murugaiyan](https://bit.ly/3YYtinf)")
st.markdown(
    "GitHub ![GitHub](https://img.icons8.com/?size=25&id=106567&format=png&color=22C3E6) [Mugesh](https://bit.ly/3CyOBTZ)"
)
st.markdown(
    "Fiverr ![Fiverr](https://img.icons8.com/?size=25&id=121864&format=png&color=22C3E6) [Mugesh](https://www.fiverr.com/sellers/mugesh_krish/edit)"
)
st.markdown(
    "Contact ![Email](https://img.icons8.com/?size=25&id=JeO1Kv9jsmLr&format=png&color=000000) [Email](mailto:mugeshmurugaiyan@gmail.com)"
)
st.markdown("---")
...
if uploaded_file:
    ...
    if text:
        ...
        wordcloud_img = WordCloud(width=width, height=height, background_color='white', max_words=200, contour_width=3,
                                  contour_color='steelblue', stopwords=all_stopwords).generate(text)
    ...
    if use_standard_stopwords or additional_stopwords:
        ...
        word_count_excl_stopwords = pd.DataFrame({'Word': words_filtered}).groupby('Word').size().reset_index(
        ...
        if st.button('Download Updated Word Count Table as CSV'):
            st.markdown(get_table_download_link(word_count_excl_stopwords, "word_count.csv", "Click Here to Download"),
                        unsafe_allow_html=True)

    # First Word Count Table (Including Custom Stopwords)
    st.subheader("Word Count Table (All Words Included)")
    word_count_incl_stopwords = pd.DataFrame({'Word': words}).groupby('Word').size().reset_index(
        name='Count').sort_values('Count', ascending=False)
    st.write(word_count_incl_stopwords)

    if st.button('Download All Word Count Table as CSV'):
        st.markdown(get_table_download_link(word_count_incl_stopwords, "all_word_count.csv", "Click Here to Download"),
                    unsafe_allow_html=True)
